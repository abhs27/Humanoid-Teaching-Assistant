def run_finger_counting_game():
   ...
   # import finger_counter  # your module for finger counting
    ...
    def draw_count_name(image, count_name):
        ...
        thickness = 3
        text_size = cv2.getTextSize(count_name, cv2.FONT_HERSHEY_SIMPLEX, font_scale, thickness)[0]
        ...
        cv2.putText(image, count_name, (x, y), cv2.FONT_HERSHEY_SIMPLEX, font_scale, (0, 0, 0), thickness, cv2.LINE_AA)
    ...
    while score < POINTS_TO_WIN:
        ...
        #finger_count = finger_counter.get_finger_count(cap)
        finger_count = fingers_counting_trails.get_finger_count_with_timer(3)
        # finger_counter.get_finger_count should return the counted fingers as an integer
        correct = False
        if finger_count == count_needed:
            correct = True
            score += 1
            SOUND_CORRECT.play()
        else:
...
# At the end of the file, allow direct run:
if __name__ == "__main__":
    run_finger_counting_game()
